...
#
# Writes: dist/crash-bandicoot-3-warped-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix crash-bandicoot-3-warped \
  --exe-name Crash_Bandicoot_3__Warped_Recompiled \
  --display-name "Crash Bandicoot 3: Warped Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Crash Bandicoot - Warped (USA).cue" \
